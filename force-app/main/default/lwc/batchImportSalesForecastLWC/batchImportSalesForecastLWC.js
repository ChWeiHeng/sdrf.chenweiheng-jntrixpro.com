...
import getInitData from '@salesforce/apex/BatchImportSalesForecastController.getInitData';
import saveData from '@salesforce/apex/BatchImportSalesForecastController.saveData';
...
import Sales_Forcast_Imp_Title from '@salesforce/label/c.Sales_Forcast_Imp_Title';
...
export default class batchImportSalesForecastLWC extends LightningElement {
	@api recordId;

	label = {
        Sales_Forcast_Imp_Title,
	...
	importFileChange(event) {
		...
		Promise.resolve(this.file)
			...
			.then(blob => {
				...
				//调用后台方法 保存数据到系统
				saveData({TargetJson: JSON.stringify(importDataContent), excelHeaderKeyToLabelMap: this.excelHeaderKeyToLabelMap, mustFillValueFields: this.mustFillValueFields,salesId: this.recordId,type: this.salesType})
				.then((result) => {
