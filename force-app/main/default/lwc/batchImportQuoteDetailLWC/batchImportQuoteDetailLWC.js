...
import getInitData from '@salesforce/apex/BatchImportQuoteDetailController.getInitData';
import saveData from '@salesforce/apex/BatchImportQuoteDetailController.saveData';
...
export default class batchImportQuoteDetailLWC extends LightningElement {
	@api recordId;
		...
		Promise.resolve(this.file)
			...
			.then(blob => {
				...
				try {
					...
					importDataContent = importData.slice(2);
				...
				//调用后台方法 保存数据到系统
				saveData({TargetJson: JSON.stringify(importDataContent), excelHeaderKeyToLabelMap: this.excelHeaderKeyToLabelMap, mustFillValueFields: this.mustFillValueFields,quoteId: this.recordId})
				.then((result) => {
