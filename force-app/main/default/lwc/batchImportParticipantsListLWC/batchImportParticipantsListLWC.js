...
import getInitData from '@salesforce/apex/BatchImportParticipantsListController.getInitData';
import saveData from '@salesforce/apex/BatchImportParticipantsListController.saveData';
...
export default class BatchImportParticipantsListLWC extends LightningElement {
	@api recordId;
	...
	importFileChange(event) {
		...
		Promise.resolve(this.file)
			...
			.then(blob => {
				...
				//调用后台方法 保存数据到系统
				saveData({TargetJson: JSON.stringify(importDataContent), excelHeaderKeyToLabelMap: this.excelHeaderKeyToLabelMap, mustFillValueFields: this.mustFillValueFields,camId: this.recordId})
				.then((result) => {
