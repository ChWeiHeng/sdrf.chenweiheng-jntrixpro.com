...
import getInitData from '@salesforce/apex/BatchImportPriceBookItemController.getInitData';
import saveData from '@salesforce/apex/BatchImportPriceBookItemController.saveData';
...
import BatchImportParticipantsListHeader from '@salesforce/label/c.BatchImportParticipantsListHeader';
...
import BatchImportPriceBookItemHeader from '@salesforce/label/c.BatchImportPriceBookItemHeader';    
...
export default class BatchImportPriceBookItemLWC extends LightningElement {
    @api recordId;
	@track numValueFieldSet;
	label = {
        BatchImportPriceBookItemHeader,
		...
		BatchImportParticipantsListHeader,
		BatchImportError1,
		BatchImportError2,
		BatchImportError3,
		BatchImportError4
	...
	connectedCallback(event){
		...
		.then(result => {
			...
			console.log('excelHeaderMap===>',JSON.stringify(this.excelHeaderMap));
			...
			this.numValueFieldSet = result.numValueFieldSet;
		...
		Promise.resolve(this.file)
			...
			.then(blob => {
				...
				try {
					...
					importDataContent = importData.slice(1);
				...
				//调用后台方法 保存数据到系统
				saveData({TargetJson: JSON.stringify(importDataContent), excelHeaderKeyToLabelMap: this.excelHeaderKeyToLabelMap, mustFillValueFields: this.mustFillValueFields,priceBookId: this.recordId,numValueFieldSet:this.numValueFieldSet})
				.then((result) => {
					// console('result====>',JSON.stringify(result));
